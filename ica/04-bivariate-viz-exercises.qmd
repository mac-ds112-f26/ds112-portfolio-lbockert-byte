...
ggplot(elections, aes(x = winner_20)) +
  geom_bar()
```
How did the republicans win by so much? Does this mean they had by far the most votes or just the most counties?
...
ggplot(elections, aes(x=repub_pct_20))+
  geom_density()
```
This data is negatively skewed. Why is the largest density of support around 75%?
...
# How does this differ from the frame for our histogram of repub_pct_20 alone? It has two variables.
...
ggplot(elections, aes(median_age, y=repub_pct_20))+
  geom_point(size=1)
```
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point()
```
A scatterplot is not an effective visualization when there is categorical data involved.
...
On average, red states had higher county support than purple states and purple states had higher county support than blue states.
...
# The colors do not match. You can not see all of the graphs.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```
...
# Fills in the different denisty plots.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r}
# What does alpha = 0.5 do? Makes the opacity of the fill different.
...
#Splits the graph into three smaller graphs.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
# Its hard to tell the difference in sizes between the three catagories.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
They shows more of the data.
-   What is one con of density plots relative to boxplots?
It is harder to tell the median and other important numbers.
...
# Historical is on the x-axis and the winner is the stacked variable.
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
```
...
A pro is that it uses the whole graph and shows percentages. A con is that the you will not get accurate counts.

#### Part b

What's your favorite bar plot from part and why?
The overlapping denisty plot because it shows a very clear comparison.
...
Missori has a lot of variability while Hawaii has very little variability.
-   What other interesting patterns do you notice?
Purple states have a larger spread of variation.
-   Does this plot prompt any other questions?
Why are some red states so low in republican support and vice versa.
...
# What info does this new geom_point() layer need to run? It runs a new set of points on top of the old one.
ggplot(elections_by_state, aes(x = repub_pct_20, y = fct_reorder(state_abbr, repub_pct_20))) + 
  geom_point(color = "red") + 
...
I think the color of the background should correlate with the color of the party.
...
Utag had the biggest change while West Virginia saw the least change. Republican support decreased in Utah and increased in Colorodo.
-   What other questions are you left with?
How did each state have different values that led to these votes.
